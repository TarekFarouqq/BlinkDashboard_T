...
export class ProductDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(this.ProductId > 0 ){
      this.productServ.GetById(this.ProductId).subscribe((res) => {
        ...
        if(this.ProductEntity.productImages && this.ProductEntity.productImages.length > 0 ){
  ...
  submitForm(){
    ...
    if(!this.ProductEntity){
      this.productServ.InsertProduct(formData).subscribe((res)=>{
        this.router.navigate(['/product/manage']);
      })
    }else{
      this.productServ.UpdateProduct(this.ProductId,formData).subscribe((res)=>{
        this.router.navigate(['/product/manage']);
      },
      (err)=>{
  ...
  getImageSrc(image: any): string | null {
    if (image && image.previewUrl) {
      return image.previewUrl;  // عرض رابط المعاينة إذا كان موجودًا
    }
